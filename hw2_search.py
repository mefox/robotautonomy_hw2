...
#    self.start = [1.2, -1.10, -0.3, 2.3, -0.2, -1.2, -2.2]
    goal = [ 0.93, -1.10, -0.2,  2.27, -0.23, -1.09, -2.23]  #### Truncated directly

    with self.env:
      self.robot.SetActiveDOFValues(self.start)

    # get the trajectory!
    traj = self.search_to_goal_breadthfirst([goal])              ########BFS
#    traj = self.search_to_goal_depthfirst([goal])               ########DFS
            ...
            for c in neighbors: #for each of the neighbors to the current node
                ...
                if self.convert_for_dict(c) not in visited_nodes:    #test if neighbor has not been visited    
                  if not self.check_collision(c,flag_coll_check):
                    ...
                    nodes.appendleft(c) #put it into the queue
	...
	for neighbor in neighbors:
		...
		if self.convert_for_dict(neighbor) not in g_score.keys(): 
			if not self.check_collision(neighbor,flag_coll_check):
				...
				dist = dist + g_score[self.convert_for_dict(neighbor)]
				#goal = np.int_(goal*100)/100.
...
#        print collision1, DOFs
    ...
    return collision2# or collision1
